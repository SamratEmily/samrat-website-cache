...
class Samrweca_Admin_Menu {

	/**
	 * Option name for storing cache settings
	 */
	const OPTION_NAME = 'samrweca_settings';
	...
	public function __construct() {
		...
		add_action('wp_ajax_samrweca_clear_cache', array($this, 'ajax_clear_cache'));
	...
	public function enqueue_admin_assets($hook) {
		if (strpos($hook, 'samrweca') === false) {
			return;
		}

		wp_enqueue_style(
			'samrweca-admin',
			SAMRWECA_PLUGIN_URL . 'assets/css/admin.css',
			array(),
			SAMRWECA_VERSION
		);
		...
		wp_enqueue_script(
			'samrweca-admin',
			SAMRWECA_PLUGIN_URL . 'assets/js/admin.js',
			array('jquery'),
			SAMRWECA_VERSION,
			true
		);

		wp_localize_script('samrweca-admin', 'samrwecaCache', array(
			'ajaxUrl'      => admin_url('admin-ajax.php'),
			'nonce'        => wp_create_nonce('samrweca_nonce'),
		...
		<div class="wrap samrat-cache-wrap">
			<div class="samrat-cache-header">
				...
				<div class="samrat-cache-version">
					...
					printf(esc_html__('Version %s', 'samrat-website-cache'), esc_html(SAMRWECA_VERSION)); ?>
				</div>
			</div>

			<?php settings_errors('samrweca_messages'); ?>
			...
			<div class="samrat-cache-dashboard">
				...
				<div class="samrat-cache-settings-container">
					<form method="post" action="options.php" class="samrat-cache-form">
						<?php
						settings_fields('samrweca_settings_group');
						do_settings_sections('samrweca-settings');
	...
	public function add_admin_bar_menu($wp_admin_bar) {
		...
		$wp_admin_bar->add_node(array(
			...
			'href'  => admin_url('admin.php?page=samrweca-settings'),
		));

		$wp_admin_bar->add_node(array(
			'parent' => 'samrweca',
			'id'     => 'samrweca-settings',
		...
		$wp_admin_bar->add_node(array(
			...
			'href'   => '#',
			'meta'   => array(
				'onclick' => 'samrwecaClearCacheFromBar(); return false;',
			),
		));
	}
	...
	public function ajax_clear_cache() {
		check_ajax_referer('samrweca_nonce', 'nonce');
